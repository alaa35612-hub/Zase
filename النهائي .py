...
def _coerce_non_negative_int(value: Any, fallback: int = 0) -> int:
    """Best-effort conversion of a value into a non-negative integer."""
...
def _resolve_recent_bar_limit(
    bars: Any, timeframe: Optional[str], timeframe_seconds: Optional[int]
) -> int:
    """Resolve a configurable recent-bar window against the active timeframe."""

    count = _coerce_non_negative_int(bars, 0)
    if count <= 0:
        return 0
    return max(0, _resolve_recent_bar_lookback(count, timeframe, timeframe_seconds))
...
class SmartMoneyAlgoProE5:
    ...
    def __init__(
        ...
        *,
        ...
        if console_inputs is not None and getattr(console_inputs, "max_age_bars", None) is not None:
            console_recent_source = getattr(console_inputs, "max_age_bars")
        else:
            console_recent_source = self.ict_inputs.recent_bars
        resolved_console_age = _resolve_recent_bar_limit(
            console_recent_source, self.base_timeframe, self.base_tf_seconds
        )
        resolved_ict_age = _resolve_recent_bar_limit(
            self.ict_inputs.recent_bars, self.base_timeframe, self.base_tf_seconds
        )
        active_recent = resolved_ict_age if self._ict_enabled() else resolved_console_age
        self.console_max_age_bars = max(0, active_recent)
        self.retracement_recent_bars: int = self.console_max_age_bars
    ...
    def processZones(self, zones: PineArray, isSupply: bool, zonesmit: PineArray) -> bool:
        ...
        while i >= 0:
            ...
            if isSupply and self.series.get("low") < botZone and self.series.get("close") > topZone:
                ...
                prev_state_flag = zonesmit.get(i)
                if prev_state_flag == 1:
                    isAlertextidm = True
                zone.set_bgcolor(touched_bg)
                zone.set_border_color(touched_border)
                zone.set_text_color(touched_text)
                next_state = 3 if prev_state_flag in (2, 3) else 2
                zonesmit.set(i, next_state)
                status = "retest" if prev_state_flag in (2, 3) else "touched"
    ...
    def _update_bar_ict(self) -> None:
        ...
        if any(math.isnan(v) for v in (high, low, close)):
            return
        if not self._ict_validate_timeframe():
            return
...
def scan_binance(
    ...
    *,
    ...
    timeframe_seconds = _parse_timeframe_to_seconds(timeframe, None)
    window = recent_window_bars
    if window is None:
        recent_source: Any = None
        if inputs is not None:
            console_inputs = getattr(inputs, "console", None)
            if console_inputs is not None and getattr(console_inputs, "max_age_bars", None) is not None:
                recent_source = getattr(console_inputs, "max_age_bars")
            else:
                ict_cfg = getattr(inputs, "ict_strategy", None)
                if ict_cfg is not None:
                    recent_source = getattr(ict_cfg, "recent_bars", None)
        resolved_window = _resolve_recent_bar_limit(recent_source, timeframe, timeframe_seconds)
        window = resolved_window if resolved_window > 0 else 1
    else:
        window = max(1, _coerce_non_negative_int(window, 1))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="تحديد عدد الشموع المستخدمة لتتبع أحدث أحداث ICT (0 لتعطيل الحد)",
    )
    ...
    if args.ict_recent_bars is not None and args.ict_recent_bars < 0:
        parser.error("--ict-recent-bars يجب أن يكون رقمًا غير سالب")
    ...
    if args.ict_recent_bars is not None:
        indicator_inputs.ict_strategy.recent_bars = args.ict_recent_bars
    if args.ict_only:
        ...
        while True:
            ...
            if not args.continuous_scan:
                print(
                    ...
                    " لتفعيل الحلقة استخدم --continuous-scan=true أو عيّن خيار التشغيل المستمر صراحة.",
                    flush=True,
                )
                break
            if args.scan_interval > 0.0:
