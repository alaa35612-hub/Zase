...
class SmartMoneyAlgoProE5:
    ...
    def __init__(
        ...
        *,
        ...
        ict_recent_override: Optional[int] = None
        resolved_override: Optional[int] = None
        if self._ict_enabled():
            try:
                ict_recent_override = max(int(self.ict_inputs.recent_bars), 0)
            except (TypeError, ValueError):
                ict_recent_override = STRUCTURE_RETRACEMENT_RECENT_BARS
            resolved_override = (
                _resolve_recent_bar_lookback(
                    ict_recent_override, self.base_timeframe, self.base_tf_seconds
                )
                if ict_recent_override and ict_recent_override > 0
                else 0
            )
            self.console_max_age_bars = max(0, resolved_override)
        if STRUCTURE_RETRACEMENT_ENABLE_RECENCY_CHECK:
        ...
        if ict_recent_override is not None:
            if ict_recent_override <= 0:
                retracement_recent = 0
            else:
                retracement_recent = resolved_override or retracement_recent
